...
export class FriDaySchedule {
    ...
    public setTuesday = (
        ...
        for (let i: number = 0; i < schedulePersonList.length; i++) {
            ...
            // 稼働上限になったので、リターン
            if (this.fri_workTime1 >= OPERATION_PEOPLE_MAX) {
                break;
            }
            // 10= time >=14
            if (
                DAY_SHIFT_0 === schedulePersonList[i].friStart &&
                DAY_SHIFT_1 <= schedulePersonList[i].friEnd
            ) {
        ...
        for (let i: number = 0; i < schedulePersonList.length; i++) {
            ...
            // 稼働上限になったので、リターン
            if (this.fri_workTime2 >= OPERATION_PEOPLE_MAX) {
                break;
            }
            // 10 <= time >=14
            if (
                DAY_SHIFT_0 <= schedulePersonList[i].friStart &&
                DAY_SHIFT_2 <= schedulePersonList[i].friEnd
            ) {
                if (schduleMap.has(schedulePersonList[i].personId)) {
                    wResult = schduleMap.get(
                        schedulePersonList[i].personId
        ...
        for (let i: number = 0; i < schedulePersonList.length; i++) {
            ...
            // 稼働上限になったので、リターン
            if (this.fri_workTime3 >= OPERATION_PEOPLE_MAX) {
                break;
            }
            // 10 <= time >=23
            if (
                DAY_SHIFT_0 <= schedulePersonList[i].friStart &&
                DAY_SHIFT_3 <= schedulePersonList[i].friEnd
            ) {
                if (schduleMap.has(schedulePersonList[i].personId)) {
                    wResult = schduleMap.get(
                        schedulePersonList[i].personId
    ...
    public getWorkTime() {
        return [this.fri_workTime1, this.fri_workTime2, this.fri_workTime3];
    }
}
