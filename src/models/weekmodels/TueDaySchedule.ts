...
export class TueDaySchedule {
    tues_workTime1: number = 0;
    tues_workTime2: number = 0;
    tues_workTime3: number = 0;

    constructor(t1: number, t2: number, t3: number) {
        this.tues_workTime1 = t1;
        this.tues_workTime2 = t2;
        this.tues_workTime3 = t3;
    }
    ...
    public setTuesday = (
        ...
        for (let i: number = 0; i < schedulePersonList.length; i++) {
            ...
            // 稼働上限になったので、リターン
            if (this.tues_workTime1 >= OPERATION_PEOPLE_MAX) {
                break;
            }
            // 10= time >=14
            if (
                DAY_SHIFT_0 === schedulePersonList[i].tueStart &&
                DAY_SHIFT_1 <= schedulePersonList[i].tueEnd
            ) {
        ...
        for (let i: number = 0; i < schedulePersonList.length; i++) {
            ...
            // 稼働上限になったので、リターン
            if (this.tues_workTime2 >= OPERATION_PEOPLE_MAX) {
                break;
            }
            // 10 <= time >=14
            if (
                DAY_SHIFT_0 <= schedulePersonList[i].tueStart &&
                DAY_SHIFT_2 <= schedulePersonList[i].tueEnd
            ) {
                if (schduleMap.has(schedulePersonList[i].personId)) {
                    wResult = schduleMap.get(
                        schedulePersonList[i].personId
        ...
        for (let i: number = 0; i < schedulePersonList.length; i++) {
            ...
            // 稼働上限になったので、リターン
            if (this.tues_workTime3 >= OPERATION_PEOPLE_MAX) {
                break;
            }
            // 10 <= time >=23
            if (
                DAY_SHIFT_0 <= schedulePersonList[i].tueStart &&
                DAY_SHIFT_3 <= schedulePersonList[i].tueEnd
            ) {
                if (schduleMap.has(schedulePersonList[i].personId)) {
                    wResult = schduleMap.get(
                        schedulePersonList[i].personId
                ...
                schedulePersonList[i].workCoun =
                    schedulePersonList[i].workCoun + 1;
                // 同時上限人数
                this.tues_workTime3 += 1;
            }
        }
    };

    public getWorkTime() {
        return [this.tues_workTime1, this.tues_workTime2, this.tues_workTime3];
    }
}
